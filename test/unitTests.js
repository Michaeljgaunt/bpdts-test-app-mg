...
describe("<<------------- API ------------->>", () => {
	...
	describe("/users/:city route - Retrieve users living in or within 50 miles of :city", () => {
		context("Called with valid input", () => {
			it("Returns response code 200 and five users (proper case input; allUsers API returning 2 users; cityUsers API returning 3 users)", async () => {
				mockAPI.get("/users").reply(200, testData.allUsers);
				mockAPI.get('/city/London/users').reply(200, testData.londonUsers);
			...
			it("Returns response code 200 and five users (mixed case input; allUsers API returning 2 users; cityUsers API returning 3 users)", async () => {
				mockAPI.get("/users").reply(200, testData.allUsers);
				mockAPI.get('/city/London/users').reply(200, testData.londonUsers);
				const res = await chai.request(app).get("/users/loNDoN")
				expect(res.body).to.have.lengthOf(5);
			...
			it("Returns response code 200 and two users (allUsers API returning 2 users; cityUsers API returning empty)", async () => {
				mockAPI.get("/users").reply(200, testData.allUsers);
			...
			it("Returns response code 200 and three users (allUsers API returning empty; cityUsers API returning 3 users)", async () => {
			...
			it("Returns response code 200 and no users (both API calls returning empty)", async () => {
		...
		context("Called with invalid input", () => {
			it("Returns response code 400 and an error (unrecognized input)", async () => {
				mockAPI.get("/users").reply(200, testData.allUsers);
				mockAPI.get('/city/London/users').reply(200, testData.londonUsers);
			...
			it("Returns response code 400 and an error (non-string input)", async () => {
				mockAPI.get("/users").reply(200, testData.allUsers);
				mockAPI.get('/city/London/users').reply(200, testData.londonUsers);
